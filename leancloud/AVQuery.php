...
class AVQuery extends AVRestClient{
  ...
  public function setLimit($int){					//设置一次查询所能接收的数量
	if ($int >= 1 && $int <= 1000){
	  $this->_limit = $int;
	}
	else{
  ...
  public function setSkip($int){					//跳过前几个结果
	$this->_skip = $int;
  }
  ...
  public function whereNear($key,$value){
	if(isset($key) && isset($value)){
	  $this->_query[$key] = array(
								  '$nearSphere' => $value
								  );
  ...
  public function withinGeoBox($key,$southwest, $northeast){
	if(isset($key) && isset($southwest) && isset($northeast)){
	  $this->_query[$key] = array(
								  '$within' => array(
  ...
  public function withinKilometers($key,$value, $maxDistance){
	if(isset($key) && isset($value)){
	  $this->_query[$key] = array(
								  '$nearSphere' => $value,
								  '$maxDistanceInKilometers' => $maxDistance
								  );
	}
	else{
	  $this->throwError('the $key and $value parameters must be set when setting a "where" query method');
  ...
  public function withinMiles($key,$value, $maxDistance){
	if(isset($key) && isset($value)){
	  $this->_query[$key] = array(
								  '$nearSphere' => $value,
  ...
  public function withinRadians($key,$value, $maxDistance){
	if(isset($key) && isset($value)){
	  $this->_query[$key] = array(
								  '$nearSphere' => $value,
								  '$maxDistanceInRadians' => $maxDistance
								  );
	}
	else{
	  $this->throwError('the $key and $value parameters must be set when setting a "where" query method');
